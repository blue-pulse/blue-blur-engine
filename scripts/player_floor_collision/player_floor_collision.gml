	...
	if (allow_collision and !on_object) {
		...
		switch (collision_mode[0]) {
			...
			case 0:		
				...
				if (!stick_to_convex) {
					var floor_distance = (!global.S2FloorCollision) ? (14) : (min(4 + abs(floor(horizontal_speed)), 14));
			...
			case 1:
				...
				// Go airborne if surface is too far away from us
				if (!stick_to_convex) {
					var floor_distance = (!global.S2FloorCollision) ? (14) : (min(4 + abs(floor(vertical_speed)), 14));
					if (found_floor[0] > floor_distance) {
						if (state == states.moving) {
							//animation_reset(0);
						}
