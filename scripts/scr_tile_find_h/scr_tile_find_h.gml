function tile_find_h(pos_x, pos_y, to_positive, tile_layer) {
	...
	var tile_width = tile_get_width(pos_y, current_tile, tile_index);
	
	// If no width found, get a tile away from the player
	if (!tile_width or tile_ignore_h(tile_index, to_positive)) {
		shift = 16;
		current_tile = tilemap_get(current_layer, (pos_x + shift * tile_direction) div 16, pos_y div 16);
		tile_index = tile_get_index(current_tile);
		tile_width = tile_get_width(pos_y, current_tile, tile_index);
	...
	var tile_distance = 0;
	if (to_positive) {
		tile_distance = ((pos_x + shift * tile_direction) & -16) + (16 - tile_width - 1) - pos_x;
	} else {
		tile_distance = pos_x - (((pos_x + shift * tile_direction) & -16) + tile_width);
	}
