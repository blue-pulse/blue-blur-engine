...
function __InputGamepadTransformSteam(_gamepadStruct)
{
    ...
    if (_system.__usingSteamworks)
    {
        with(_gamepadStruct)
        {
            var _gamepadIsVirtual = (INPUT_ON_WINDOWS and __xinput);
            ...
            if (INPUT_ON_LINUX)
            {
                ...
                {
                    if ((gamepad_get_description(_i) == "Valve Streaming Gamepad") or __InputStringContains(gamepad_get_guid(_i), "03000000de280000fc11", "03000000de280000ff11"))
                    {
                        _gamepadHandleIndex++;
            ...
            if ((INPUT_ON_MACOS and (__guid == "030000005e0400008e02000001000000"))
            or  (INPUT_ON_LINUX and (__guid == "03000000de280000ff11000001000000")))
            {
            ...
            if not (_gamepadIsVirtual and is_numeric(__steamHandle) and (__steamHandle > 0))
            {
            ...
            else
            {
                ...
                if not (is_numeric(_steamType) and (_steamType >= 0))
                {
    ...
    if ((__vendor != "de28") and ds_map_exists(_steamInputTypeIgnoreMap, __type))
    {
