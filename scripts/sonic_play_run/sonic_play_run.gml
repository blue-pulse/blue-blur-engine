function sonic_play_run() {
	...
	if (current_speed > 0 and current_speed < 1.5) {
		anim_speed = clamp(current_speed, 0.67, 1);
	...
	// Jog
		...
		anim_speed = map(current_speed, 4, 6.1, 1, 2);
		animation_play(anim_jog, anim_speed);
	}

	// Run
	else if (current_speed >= 4.5 and current_speed < 6.0) {
		anim_speed = map(current_speed, 6.1, 10, 1.5, 2.5);
		animation_play(anim_run, anim_speed);
	}

	// Sprint
	else if (current_speed >= 6.0 and current_speed < 10.0) {
		var anim_speed = map(current_speed, 6.1, 10, 1.5, 2.5);
		animation_play(anim_sprint, anim_speed);
	}

	// Peelout
	else if (current_speed >= 10.0) {
		anim_speed = map(current_speed, 10, speed_cap, 1.35, 2.5);
